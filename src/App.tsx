...
import {
  ...
  FULL_TILE_DIMS_PX,
  FULL_TILE_DIM_COUNT,
  MAX_LEVEL_ENTRANCE_ID,
  NAV_CONTAINER,
  TILE_QUADRANT_DIMS_PX,
...
import {
  getDefaultRenderTextures,
  getGraphicFromChunkCode,
  INVERT_CACHE,
  isDefaultChunkCode,
} from './rom-mod/tile-rendering/texture-generation';
...
function App() {
   ...
   * Context
   */
  const { loadRomFromArrayBuffer, romBuffer, romData, setRomData } =
    useContext(RomContext);
  ...
  const [curSelectedObject, setCurSelectedObject] =
    useState<LevelObject | null>(null);
  const [interactiveSprite, setInteractiveSprite] = useState<
    Sprite | undefined
  >(undefined);
   ...
   * Effects
   */
  useEffect(initPixieAppEffect, []);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(interactiveSpriteEffect, [
    curSelectedObject,
    romData,
    interactiveSprite,
  ]);
  ...
  const rerenderPages = (noCache: boolean = false) => {
    ...
    if (hasPixieError(pixiApp) || hasRomError(romData)) {
      return;
    }
    const levelRef = getLevelByOffsetId(romData.levels, curLevelId);
    if (hasCustomError<Level | undefined>(levelRef, 'No levelRef found')) {
      return;
    }
    if (noCache) {
    ...
    console.debug(
      `rerenderPages completed in ${performance.now() - rerenderPerf} ms`
    );
  };
  ...
  const _reapplySelect = () => {
    ...
    ScreenPageData.applyEffectToSingleObject(
      curSelectedObject.uuid,
      screenPageData,
      'inverted'
    );
  };
  ...
  window.spriteClicked = (e: any) => {
    ...
    const foundScreenPages = screenPageData.filter(
      (sp) => sp.screenPageId === spid
    );
    if (foundScreenPages.length === 1) {
      ...
      if (foundObjects.length > 1) {
        ...
        selectedObject = foundObjects.filter(
          (x) => x.zIndex === highestLayer
        )[0];
      }
   ...
   */
  const fileOpened = (e: ChangeEvent<HTMLInputElement>) => {
    if (!pixiApp) {
    ...
    if (target.files.length !== 1) {
      console.error(
        `Incorrect number of files uploaded: ${target.files.length}`
      );
      return;
    }
    ...
    file
      ...
      .then((result) => {
        ...
        const interactiveSprite = Sprite.from(
          textureCache[WHITE_SQUARE_RENDER_CODE]
        );
  ...
  const tileTest = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const chunkTest = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="App">
      ...
      <ButtonControls
        ...
        loading={loading}
        replaceAllChunks={replaceAllChunks}
        redo={redo}
   ...
   * Effect Hoists
   */
  function initPixieAppEffect() {
    const newPixiApp = generatePixiApp();
    curLevelId = 0x0;
    if (!newPixiApp) {
      console.error('PixiApp failed to initialize for graphics generation');
    } else {
  ...
  function interactiveSpriteEffect() {
    if (
      hasSpriteError(interactiveSprite) ||
      hasPixieError(pixiApp) ||
      hasRomError(romData)
    ) {
      return;
    }
    ...
    interactiveSprite!.on('pointerdown', (event: any) => {
      window.spriteClicked(event);
      const didStart = handleDragStart(
        pixiApp!,
        event.data.global,
        curSelectedObject,
        screenPageData
      ...
      if (didStart) {
        ...
        if (levelData) {
          ...
          console.error(
            'No level data found when trying to start drag!',
            levelData
          );
          cachedLevelData = '';
        }
